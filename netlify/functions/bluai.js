...
    try {
        // Added User-Agent header to bypass potential 503 firewalls
...
    const userPrompt = requestBody.prompt;
    // 🛑 FIX: Get history from the request body for chat memory
    const history = requestBody.history || [];

    // 🛑 Check for Trivial/Ending Prompts (Your existing logic)
...
    // 🛑 OPTIMIZATION: Await the cached promise instead of fetching every time
    contextToInject = await knowledgePromise;
...
Example 2: A user says, "I can't log in to my account." The AI MUST immediately respond with Rule 42 (Escalation).
...
T  he AI MUST NOT include details about payment, pickup, or international shipping in this same response.
...
* Example: "Delivery takes 3-5 business days.---BREAK---Returns are only for damaged items."
...
    LITERAL BLOCK END
    
    56. Follow-Up Template (ABSOLUTE FINAL): After delivering the CriticalEvent Literal Response (Rule 55), you MUST immediately follow it with the literal string **---BREAK---** and the following text template, filling in the necessary transitional phrase:
    
...
// --- Start of NEW API Call Logic (REPLACEMENT) ---
    ...
    // 1. 🛑 FIX: Initialize the model with the correct model name and system instruction
    const model = ai.getGenerativeModel({
        model: "gemini-2.5-flash-lite", // 🚀 As requested: The fastest model
        systemInstruction: brandPersona
    });

    // 2. 🛑 FIX: Start a chat session using the provided history
    ...
            
            // 3. 🛑 FIX: Use chat.sendMessage() to send the RAG-infused prompt
    ...
            // Check for 503 error to retry (Your existing logic is good)
      t     if (error.message.includes('503') && attempt < MAX_RETRIES) {
    ...
    // 4. Get the response text from the 'result' object
    const response = result.response;
    const rawResponseText = response.text(); // Get raw text for history

    // 5. Process the text for display (replace ---BREAK---)
    let finalResponseText = rawResponseText.replace(/---BREAK---/g, '\n\n');

    // 6. 🛑 FIX: Create the new history array to send back to the client
    const updatedHistory = [
        ...history,
        { 
            role: "user", 
            // Use the *original* user prompt for history, not the RAG-filled one
            parts: [{ text: userPrompt }] 
        },
        { 
            role: "model", 
            // Use the *raw* model response for history, before formatting
